...
const nodemailer = require('nodemailer'); //for sendting otp to any user


const transporter = nodemailer.createTransport({
    service: 'gmail',
    auth: {
        user: 'akshaychaman2004@gmail.com',  //for sending otp to user from company email
        pass: 'jkslwvyiritqdxfa'
    }
})
...
router.post('/login', async (req, res, next) => {
    try {
        ...
        if (!isMatch) {
            return res.status(400).json(createResponse(false, 'Invalid credentials')); //invalid password
        }
...
router.post('/sendotp', async (req, res) => {
    try {
        ...
        const otp = Math.floor(100000 + Math.random() * 900000); //otp for 6 digits

        const mailOptions = {
            from: 'akshaychaman2004@gmail.com',
            to: email,
